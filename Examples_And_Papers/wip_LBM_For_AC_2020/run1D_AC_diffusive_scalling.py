...
for dtn in np.arange(1,7)  : # (start, stop, num)
    lbdt = 1./(4**dtn)
    print(domain_size * 2**dtn)
    tc = 100
    def getXML(**kwars):
            ...
            fname = prefix+"run"
            CLBc.addGeomParam('nx', domain_size * 2**dtn)
            ...
            CLBc.addBox(nx=int(domain_size/2)* 2**dtn)
           ...
           # CLBc.addModelParam("Init_PhaseField",-0.9,'down' )
            ...
            CLBc.addSolve(iterations=50)
            CLBc.addVTK()
            print(tc/lbdt-50)
            CLBc.addSolve(iterations=tc/lbdt-50)
            CLBc.addVTK()
    ...
    for tmp in tmps:
        ...
        for field_name in ['PhaseField']:
            row[field_name] = vti.get(field_name)[0,::dtn]
...
reference = L2[-1]['LBM_final'][:,::2**(np.arange(1,7)[-1]-1)]
# plt.figure()
...
for i in range(len(L2)):
    ...
    final = L2[i]['LBM_final'][:,::2**(np.arange(1,7)[i]-1)]
