...
for dtn in np.arange(1,4)  : # (start, stop, num)
    lbdt = 1./(2**dtn)
...
"""
		...
		y = Solver$Geometry$Y		
		Solver$Fields$phi0[] = sin(x)
		Solver$Fields$ux[] = 0*x
		Solver$Fields$uy[] = 0*x        
...
""".format(domain_size=domain_size)
    ...
    os.system("cd %s && ~/projekty/TCLB/tools/sirun.sh advection_diffusion2D_fields   ./run.xml"%d0)
