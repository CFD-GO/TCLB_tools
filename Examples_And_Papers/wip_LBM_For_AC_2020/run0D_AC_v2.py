...
################################################
# CONFIG
tc = 100                     # number of timesteps for dt=1 aka Time
domain_size0 = 2           # initial size of the domain
...
#    reference = L2[-1]['LBM_field_all'][::2**n,::2**n]
    ...
    reference =  AC0D(tc, lambda_ph0, 0.1)
    ...
    y = dt**1
    y = y / y[0] * L2[0]['err_L2']
    ax1.loglog(dt,y, label=r'${x}$', linewidth=2)
    
    y = dt**2
    y = y / y[0] * L2[0]['err_L2']
    ax1.loglog(dt,y, label=r'${x^2}$', linewidth=2)
    
    
    
    ax1.loglog(L2dr.LBMdt, L2dr.err_L2, linestyle="", color='black', marker='o', markersize=10)
    ax1.set_xscale('log', base=2)
    ax1.legend()
    ax1.grid(True)
    ax1.set(xlabel=r'$\epsilon_t$', ylabel=r'$L_2(\phi(\delta x), \phi(\delta x_{min})$')
