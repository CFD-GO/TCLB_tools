...
r0 = 50.5  # infectious radius
beta_sir = 3.01  # the average number of contacts per person per time
gamma_sir = 1/0.2  # 1 over days to recovery  # TODO: --> high gamma_sir non conservative scheme :/
...
def SIR_1D_FD(S_IC, I_IC, R_IC, nx, r0, beta_sir, gamma_sir, total_time):
    ...
    for n in range(nt):  # iterate through time
        qSI = dt * beta_sir * S * I / N

        C = C0 * S * dt / N
        lap_I = (I[l_ind] - 2 * I[c_ind] + I[r_ind]) / dx ** 2
        qSI_spatial = C * lap_I
        qSI_spatial = np.zeros(nx)
...
S, I, R = SIR_1D_FD(S_IC, I_IC, R_IC, nx, r0, beta_sir, gamma_sir, total_time)
