...
def read_data_from_toolbox():
    ny = int(150*4)  # take the dense sampling

    filepath_csv = os.path.join(main_folder, 'from_toolbox', f'Temperature_{ny}_clmax_{clmax}_Re_{Re}_Pr_{Pr}.csv')
    T_toolbox = np.loadtxt(filepath_csv, delimiter=',', skiprows=1)

    filepath_csv = os.path.join(main_folder, 'from_toolbox', f'ux_{ny}_clmax_{clmax}_Re_{Re}_Pr_{Pr}.csv')
