...
rho = 1.
kin_viscs = np.array([0.1])
effdiams = np.array([15, 31, 61, 121])
...
for k in range(n_kin_visc):
    for d in range(n_diam):
        ...
        uz_ibb_mse[k, d], uz_ibb_L2[k, d] = calculate_error_norms(case_folder)
        # mse, L2 = calculate_error_norms(case_folder)
        # uz_ibb_mse[d] = mse
        # uz_ibb_L2[d] = L2

        case_folder = f'bb_PoiseuillePipe_CM_HIGHER_nu_{kin_viscs[k]}_effdiam_{effdiams[d]}'
        uz_bb_mse[k, d], uz_bb_L2[k, d] = calculate_error_norms(case_folder)

        print(f"uz_bb_mse={uz_bb_mse[d]:.2e} for v{kin_viscs[k]}_effdiam_{effdiams[d]}")
        print(f"uz_bb_L2={uz_bb_L2[d]:.2e} for v{kin_viscs[k]}_effdiam_{effdiams[d]}")
        print(f"uz_ibb_mse={uz_ibb_mse[d]:.2e} for v{kin_viscs[k]}_effdiam_{effdiams[d]}")
        print(f"uz_ibb_L2={uz_ibb_L2[d]:.2e} for v{kin_viscs[k]}_effdiam_{effdiams[d]}")
...
def make_plot_for_given_viscosity(_v):
    ...
    if not os.path.exists('plots'):
        os.makedirs('plots')
    fig_name = f'plots/grid_convergence_Poiseuille_anal_vs_lbm_nu{eat_dots_for_texmaker(kin_viscs[_v])}.png'
    ...
    initial_error_2nd = 0.85*min((max(uz_ibb_L2), max(uz_bb_L2)))
...
def make_plot_for_all_viscosities():
    ...
    if not os.path.exists('plots'):
        os.makedirs('plots')
    fig_name = f'plots/grid_convergence_Poiseuille_pipe.png'
