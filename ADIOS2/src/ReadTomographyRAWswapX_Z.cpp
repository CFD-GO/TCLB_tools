...
int process(int argc, char*argv[]){
    ...
    ITYPE NX = atoi(argv[3]);
    ITYPE NY = atoi(argv[4]);
    ITYPE NZ = atoi(argv[5]);
    ITYPE rY = atoi(argv[9]);
    ...
    std::cout << "Wiil read  " << rY << " Y slices" << std::endl;
    ...
    std::vector<DTYPE> buffer(NX*NZ);

    std::vector<FTYPE> masked(NX*NZ);
    

    std::cout << "Byte size: " <<  sizeof(DTYPE) << std::endl;
    std::cout << "Expected filesize: (ls -la) " <<  NX*NY*NZ*sizeof(DTYPE) << std::endl;

    DTYPE * rbuffer = buffer.data();

    for (ITYPE slice = 0; slice < NZ; slice++) {
        rf.read((char*)(rbuffer + NX*slice), NX*sizeof(DTYPE));
        if (!rf)
            std::cout << "error: only " << rf.gcount() << " could be read" << std::endl;

        rf.seekg (slice*NX*NY*sizeof(DTYPE), rf.beg);
    }
    ...
    try
    {
        ...
        std::vector<unsigned long> slice_shape({NZ,NX});
        adios2::Variable<FTYPE> varGlobalArray = io.DefineVariable<FTYPE>("GlobalArray", slice_shape);
        ...
        for (;step < rY; step++)
        {
            ...
            rf.seekg ((NX*step)*sizeof(DTYPE), rf.beg);

            for (ITYPE slice = 0; slice < NZ; slice++) {
              //  std::cout << "pointer at " << rf.tellg() << std::endl;
                rf.read((char*)(rbuffer)+NX*slice*sizeof(DTYPE), NX*sizeof(DTYPE));
                if (!rf){
                    std::cout << "error: only " << rf.gcount() << " could be read" << std::endl;
                    writer.Close();
                    return 1;
                ...
                rf.seekg ((NX*NY-NX)*sizeof(DTYPE), rf.cur);
            }
            ...
            varGlobalArray.SetSelection(adios2::Box<adios2::Dims>( {0,0}, slice_shape));
...
int main(int argc, char *argv[])
{
    ...
    if (10 > argc) {
        std::cout << "Usage: ReadTomographRAW path_to_filename.raw path_to_adios.xml NX NY NZ BYTES SIGNED(0 or 1) RelativeThreshold NoReadY" << std::endl;
        return 1;
    } 
